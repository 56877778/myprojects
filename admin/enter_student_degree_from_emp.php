...
' );
$faild_form = $connect->execute_query('SELECT std_id, std_fname, std_lname, exam_id,marks,remarks,is_failed
FROM (
student
JOIN examscore ON student.std_id = examscore.student_id
AND  '.$level.' = examscore.level_id
...
$count_student_success_avaliable = $update_form->num_rows;
$count_student_failed_avaliable = $faild_form->num_rows;
...
echo "$count_student_failed_avaliable";
?>
...
<div class="container">
  <div class="row">
    ...
    <div class="text-center">
         ...
         <img id="imgStudent0" src="upload\13.jpg" class="img-rounded img-responsive img-thumbnail">
       </div>
          <div class="col-sm-4 pull-right text-center ">
...
<div class="panel-body wonder">
       <?php     if( $count_student_failed_avaliable>=1 || $count_last_student>=1){
echo   '  <div class="row text-center">
<div><a href="enter_student_degree_was_failed_femp.php?exam_id='.$examId.'&emp_id='.$_SESSION['emp_id'].'" class="btn btn-sm btn-danger" ><span class="badge">'.$count_student_failed_avaliable.'</span> عرض الطلاب الباقون</a>
<a href="enter_student_degree_was_last_femp.php?exam_id='.$examId.'&emp_id='.$_SESSION['emp_id'].'" class="btn btn-sm btn-danger"  ><span class="badge">'.$count_last_student.'</span> جديد  </a>
...
<br>
  ...
  <br>
             ...
             <br>
             <?php     if($count_student_success_avaliable<1 & $count_student_failed_avaliable<1 ){
                   echo ' <h2 class="text-center ">  <strong> كشف إضافة درجات      </strong> </h2>'; }
                   else{
                       echo '  <h2 class="text-center ">  <strong> تعديل الدرجات     </strong> </h2>'; }
       ...
       <form class="form-horizontal " id="add_degree_exam_form" role="form"  data-toggle="validator">
         ...
         <?php     if($count_student_success_avaliable<1 & $count_student_failed_avaliable<1 ){
               echo '  <input type="hidden" name="operation" value= "Add" id="exam_id">'; }
               else{
               ...
               ?>
                  <table dir="rtl" id="example"class="table table-hover  table-bordered table-responsive ">
                    ...
                    <tbody>
                      ...
                      <?php
                      if($count_student_success_avaliable<1){
                    //    echo '  <input type="hidden" name="operation" value= "Add" id="exam_id">';


/////////عندما لم يوجد درجات مدخلة للمادة
                      while($row = mysqli_fetch_object($empty_form))
                        # code...
                      {
   ...
   <input type="hidden" id="std_id" name="std_ID[]" value="'.$student_id.'"  readonly="true"></input>
                         ...
                         <td><label class="label-control">'. $student_fname.'  '. $student_lname.' </lable>  </td>

                          <td><input  type="number" name="marks[]"  id="marks2"
                            max="'.$max_degree.'"   class="form-control "
                                onkeyup="test_degree(this,'.$min_degree.',0)"> </td>
                  ...
                  echo '   </tbody>
                              ...
                              class="btn-lg btn btn-primary" value="إنشاء" >
...
<input type="hidden" id="std_id" name="std_ID[]" value="'.$student_id.'"  readonly="true"></input>
                  ...
                  ';
                    ...
                    if($row->is_failed==1){
                      echo '   <td ><input  type="number" value="'.$marks.'" name="marks[]"  onkeyup="test_degree(this,'.$min_degree.','. $row->marks.')"     max="'.$max_degree.'"
                      ...
                      echo '   <td ><input  type="number" value="'.$marks.'" name="marks[]"  onkeyup="test_degree(this,'.$min_degree.','. $row->marks.')"
